...
public class PlayHandler extends SimpleChannelUpstreamHandler {
      ...
      try {
        ...
        boolean raw =
            Play.pluginCollection.rawInvocation(
                request, response, null, RenderArgs.current(), null);
        if (raw) {
  ...
  private class Netty3Invocation extends Invocation {
    ...
    public void execute() {
      ...
      // Check if the size was exceeded before re-rendering, so we can render the error
  ...
  private void writeResponse(
    ...
    byte[] content =
        nettyRequest.getMethod().equals(HttpMethod.HEAD) ? new byte[0] : response.out.toByteArray();
  ...
  private void copyResponse(
    ...
    if (file != null && file.isFile()) {
      ...
      if (nettyResponse.getStatus().equals(HttpResponseStatus.NOT_MODIFIED)) {
        ...
        fileService.serve(
            file, nettyRequest, nettyResponse, ctx, request, response, ctx.getChannel());
      }
    } else if (is != null) {
  ...
  private void printResponse(
      ChannelHandlerContext ctx, HttpResponse nettyResponse, String errorHtml) {
  ...
  private void serveLocalFile(
    ...
    if (nettyResponse.getStatus().equals(HttpResponseStatus.NOT_MODIFIED)) {
      ...
      fileService.serve(
          localFile, nettyRequest, nettyResponse, ctx, request, response, e.getChannel());
    }
  }
