...
Unlike with Play1, RePlay does not make use of the `play` command line tool.
...
The projects in `replay-tests` may also give some idea of how to set up certain features (like the use of Kotlin).
...
Make controllers methods (actions) non-static, and subclass from `RebelController`.
