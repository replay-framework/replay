...
public class TagFileSpacesTest {
  ...
  private void doLineFeedTests(TemplateSourceRenderer sr) {
    ...
    assertThat(sr.renderSrc("beforeTag-#{simpleTag 'data'/}-afterTag", args))
        .isEqualTo("beforeTag-[from tag: data]-afterTag");
    assertThat(sr.renderSrc("beforeTag-#{tagWithScriptBlock 'data'/}-afterTag", args))
        .isEqualTo("beforeTag-[from tag: data]-afterTag");
    assertThat(sr.renderSrc("beforeTag-#{tagWithScriptBlockAndComment 'data'/}-afterTag", args))
        .isEqualTo("beforeTag-[from tag: data]-afterTag");
  }
}
