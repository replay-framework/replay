...
public class JavaExtensionTest {
  ...
  private TemplateSourceRenderer createSourceRenderer() {
    TemplateSourceRenderer sr =
        new TemplateSourceRenderer(
            new GTTemplateRepoBuilder()
                .withTemplateRootFolder(new TemplateRootFolder())
                .withGTJavaExtensionMethodResolver(
                    methodName -> {
  ...
  public void testStaticNameClashProblem() {
    ...
    args.put("a", "ab&c");
    assertThat(sr.renderSrc("${org.apache.commons.text.StringEscapeUtils.escapeHtml4(a)}", args))
        .isEqualTo("ab&amp;c");
  }
  ...
  public void testStrangeFirstTimeGroovyObjectUsageParamsIssue() {
    ...
    // Do it one time
    assertThat(sr.renderSrc("${o.methodNameThatCanCollideWithGroovyObjectSupport(id)}", args))
        .isEqualTo("method:methodNameThatCanCollideWithGroovyObjectSupport-arg:1");
    // Do it again
    assertThat(sr.renderSrc("${o.methodNameThatCanCollideWithGroovyObjectSupport(id)}", args))
        .isEqualTo("method:methodNameThatCanCollideWithGroovyObjectSupport-arg:1");
    ...
    assertThat(
            sr.renderSrc(
                "${o.methodNameThatCanCollideWithGroovyObjectSupport(id)} - ${o.methodNameThatCanCollideWithGroovyObjectSupport(id)}",
                args))
        .isEqualTo(
            "method:methodNameThatCanCollideWithGroovyObjectSupport-arg:1 - method:methodNameThatCanCollideWithGroovyObjectSupport-arg:1");
  }
}
