...
public class GroovyInlineTags {
  ...
  public static String _elseif(int index, CALL f) {
    ...
    switch (f) {
      case START:
        s.append("if(play.templates.TagContext.parent().data.get('_executeNextElse') && attrs")
            .append(index)
            .append("['arg']) {");
        break;
      case END:
    ...
    switch (f) {
      case START:
        ...
        s.append("if(attrs")
            .append(index)
            .append(
                "['items']) { play.templates.TagContext.parent().data.put('_executeNextElse', false);");
        s.append("_iter")
            .append(index)
            .append(" = attrs")
            .append(index)
        ...
        s.append("setProperty(attrs")
            .append(index)
            .append("['as']+'_isLast', !_iter")
            .append(index)
            .append(".hasNext());");
        s.append("setProperty(attrs")
            .append(index)
            .append("['as']+'_isFirst', _")
            .append(index)
            .append("_i == 1);");
        s.append("setProperty(attrs")
            .append(index)
            .append("['as']+'_parity', _")
            .append(index)
            .append("_i%2==0?'even':'odd');");
        break;
      case END:
        s.append("};");
        s.append(
            "} else { play.templates.TagContext.parent().data.put('_executeNextElse', true); }");
        break;
    }
    return s.toString();
  }
