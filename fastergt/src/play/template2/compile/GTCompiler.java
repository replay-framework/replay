...
public class GTCompiler {
  ...
  public GTCompiler(
      GTTemplateRepo templateRepo,
      GTPreCompilerFactory preCompilerFactory,
      boolean storeSourceToDisk) {
  ...
  public static class CompiledTemplate {
    ...
    public CompiledTemplate(
        String templateClassName, GTJavaCompileToClass.CompiledClass[] compiledJavaClasses) {
  ...
  public CompiledTemplate compile(GTTemplateLocation templateLocation) {
    // precompile it
    GTPreCompiler.Output precompiled =
        preCompilerFactory.createCompiler(templateRepo).compile(templateLocation);
    ...
    if (srcDestFolder != null && storeSourceToDisk) {
      ...
      String javaFileName =
          precompiled.javaClassName.replace(GTPreCompiler.generatedPackageName + ".", "") + ".java";
      File file = new File(folder, javaFileName);
      writeContent(precompiled.javaCode, file, "utf-8");
      String groovyFileName =
          precompiled.groovyClassName.replace(GTPreCompiler.generatedPackageName + ".", "")
              + ".groovy";
    ...
    // compile groovy
    GTJavaCompileToClass.CompiledClass[] groovyClasses =
        new GTGroovyCompileToClass()
            .compileGroovySource(
                templateLocation, precompiled.groovyLineMapper, precompiled.groovyCode);
    ...
    GTJavaCompileToClass.CompiledClass[] compiledJavaClasses =
        new GTJavaCompileToClass(cl).compile(precompiled.javaClassName, precompiled.javaCode);
    ...
    return new CompiledTemplate(
        precompiled.javaClassName,
        allCompiledClasses.toArray(new GTJavaCompileToClass.CompiledClass[] {}));
  }
}
