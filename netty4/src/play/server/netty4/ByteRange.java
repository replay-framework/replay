...
class ByteRange {
  ...
  ByteRange(
    ...
    if (includeHeader) {
      header =
          ByteRangeInput.makeRangeBodyHeader(
                  ByteRangeInput.DEFAULT_SEPARATOR, contentType, start, end, fileLength)
              .getBytes(UTF_8);
    } else {
      header = new byte[0];
    }
  ...
  int fill(byte[] into, int offset) {
    ...
    int count = 0;
    for (;
        offset < into.length && servedHeader < header.length;
        offset++, servedHeader++, count++) {
    ...
    if (offset < into.length) {
      try {
        ...
        long maxToRead =
            remaining() > (into.length - offset) ? (into.length - offset) : remaining();
        ...
        if (read < 0) {
          throw new UnexpectedException(
              "error while reading file : no more to read ! length="
                  + raf.length()
                  + ", seek="
                  + (start + servedRange));
        }
