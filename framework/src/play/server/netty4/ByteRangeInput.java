...
public class ByteRangeInput implements ChunkedInput<ByteBuf> {
    ...
    public ByteRangeInput(String filePath, RandomAccessFile file, String contentType, HttpRequest request) throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("Invoked ByteRangeServer, found byteRanges: {} (with header Range: {}) on file {}",
                    Arrays.toString(byteRanges), request.headers().get("range"), filePath);
        }
    }
    ...
    private void initRanges() {
        try {
            ...
            for(int i = 0; i < reducedRanges.length; i++) {
                ...
                byteRanges[i] = new ByteRange(filePath, file, range[0], range[1], fileLength, contentType, reducedRanges.length > 1);
            }
