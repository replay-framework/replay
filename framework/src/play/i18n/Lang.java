...
public class Lang {
  ...
  @Deprecated static final ThreadLocal<String> current = new ThreadLocal<>();
   ...
   * @deprecated This method depends on current HTTP request, and thus can be used only from web
   *     controllers.
   */
  @Deprecated
  ...
  /**
   * Clears the current language - This wil trigger resolving language from request if not manually
   * set.
   */
  ...
  public static void change(String locale, Response response) {
    ...
    if (set(closestLocale)) {
      if (response != null) {
        ...
        response.setCookie(
            Play.configuration.getProperty("application.lang.cookie", "PLAY_LANG"),
            locale,
            null,
            "/",
            null,
  ...
  /**
   * Given a set of desired locales, searches the set of locales supported by this Play! application
   ...
   *
   * <ol>
   *   <li>if a <b>PLAY_LANG</b> cookie is set, use this value
   *   <li>if <b>Accept-Language</b> header is set, use it only if the Play! application allows it.
   *       <br>
   *       supported language may be defined in application configuration, eg :
   *       <em>play.langs=fr,en,de)</em>
  ...
  /**
   * @return the default locale if the Locale cannot be found otherwise the locale associated to the
   *     current Lang.
   */
