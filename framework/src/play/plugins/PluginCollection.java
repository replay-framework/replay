 ...
 *
 * <p>Loading/reloading/enabling/disabling is handled here.
 *
 * <p>This class also exposes many PlayPlugin-methods which when called, the method is executed on
 * all enabled plugins.
 *
...
public class PluginCollection {
  ...
  public void loadPlugins() {
    ...
    SortedSet<PluginDescriptor> pluginsToLoad =
        pluginsFiles
            .stream()
  ...
  List<ClasspathResource> getPlayPluginFiles() {
    String[] pluginsDescriptorFilenames =
        Play.configuration.getProperty("play.plugins.descriptor", "play.plugins").split(",");
    List<ClasspathResource> pluginDescriptors =
        Arrays.stream(pluginsDescriptorFilenames)
        ...
        .forEach(
            plugin -> {
              try {
                ...
                logger.error(
                    "Failed to handle action invocation exception by plugin {}",
        ...
        .forEach(
            plugin -> {
              try {
                ...
                logger.error(
                    "Failed to handle job invocation exception by plugin {}",
                    plugin.getClass().getName(),
                    ex);
              }
            });
  ...
  public void onActionInvocationResult(
    ...
    getEnabledPlugins()
        .forEach(
            plugin ->
                plugin.onActionInvocationResult(
