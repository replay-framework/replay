...
public class RenderBinary extends Result {
  ...
  public RenderBinary(
      InputStream is, String name, long length, String contentType, boolean inline) {
    this.is = is;
    this.name = name;
    this.contentType = contentType;
    this.inline = inline;
  ...
  /**
   * Send a file as the response. Content-disposition is set to attachment, name is taken from
   * file's name
   *
   * @param file readable file to send back
   */
  ...
  private void addContentDispositionHeader(Response response) throws UnsupportedEncodingException {
    if (name == null) {
      ...
      response.setHeader(
          "Content-Disposition", String.format(contentDisposition, dispositionType(), name));
    } else {
      ...
      response.setHeader(
          "Content-Disposition",
          String.format(
              contentDisposition, dispositionType(), encoder.encode(name, encoding.name())));
    }
  }
  ...
  private static void renderInputStream(InputStream is, long length, Response response)
      throws IOException {
