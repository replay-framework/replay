...
public class RoutesParser {
  ...
  /**
   * Parse a route file. If an action starts with <i>"plugin:name"</i>, replace that route by the
   * ones declared in the plugin route file denoted by that <i>name</i>, if found.
   */
  ...
  private void assertDoesNotContain(
      ClasspathResource routesFile, String content, String substring) {
    if (content.contains(substring)) {
      throw new IllegalArgumentException(
          "Routes file " + routesFile + " cannot contain " + substring);
    }
  }
    ...
    for (String line : content.split("\n")) {
      ...
      try {
        ...
        if (route.action.startsWith("module:")) {
          throw new IllegalArgumentException(
              String.format("Modules are not supported anymore (found route '%s')", route.action));
        } else {
          routes.add(
              new Route(
                  route.method,
