 ...
 *
 * <p>Get latest BBC World news as a RSS content
 *
 * <pre>
...
public class WSAsync implements WSClient {
  ...
  public WSAsync() {
    ...
    if (isNotEmpty(keyStore)) {
      logger.info(
          "Keystore configured, loading from '{}', CA validation enabled : {}",
          keyStore,
          CAValidation);
    ...
    // When using raw urls, AsyncHttpClient does not encode the params in url. This means we can/must encode it (with
    // the correct encoding) before passing it to AsyncHttpClient.
  ...
  private Optional<ProxyServer> buildProxy() {
    ...
    String proxyPort =
        Play.configuration.getProperty("http.proxyPort", System.getProperty("http.proxyPort"));
    String proxyUser =
