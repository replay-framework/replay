...
public class HTTP {
  ...
  private static Optional<Charset> parseEncoding(String[] contentTypeParts) {
    ...
    if (contentTypeParts.length >= 2) {
      ...
      if (encodingInfoParts.length == 2
          && "charset".equalsIgnoreCase(encodingInfoParts[0].trim())) {
        // encoding-info was found in request
        String _encoding = encodingInfoParts[1].trim();

        if (isNotBlank(_encoding)
  ...
  private static final Map<String, String> lower2UppercaseHttpHeaders =
      initLower2UppercaseHttpHeaders();
