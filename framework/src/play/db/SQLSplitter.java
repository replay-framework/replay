...
public class SQLSplitter implements Iterable<CharSequence> {
   ...
   * @return The index that skips past the quote starting at start. If the quote does not start at
   *     that point, it simply returns start.
   */
      ...
      case '$':
        {
          ...
          for (; s.charAt(quoteEnd) != '$'; ++quoteEnd) if (quoteEnd >= s.length()) return quoteEnd;
          int i = quoteEnd + 1;
          while (i < s.length()) {
            if (s.charAt(i) == '$') {
              boolean match = true;
  ...
  static int consumeTillNextLine(CharSequence s, int start) {
    while (start < s.length() && !isNewLine(s.charAt(start))) ++start;
    while (start < s.length() && isNewLine(s.charAt(start))) ++start;
    return start;
  }

  static boolean isNext(CharSequence s, int start, char c) {
    if (start + 1 < s.length()) return s.charAt(start + 1) == c;
    return false;
  }
   ...
   * @return The index that skips past the comment starting at start. If the comment does not start
   *     at that point, it simply returns start.
   */
  static int consumeComment(CharSequence s, int start) {
    if (start >= s.length()) return start;
    switch (s.charAt(start)) {
      case '-':
        if (isNext(s, start, '-')) return consumeTillNextLine(s, start + 2);
        else return start;
      ...
      case '/':
        if (isNext(s, start, '*')) {
          ...
          while (start < s.length()) {
            if (s.charAt(start) == '*') {
              ++start;
              if (start < s.length() && s.charAt(start) == '/') return start + 1;
            } else ++start;
          }
        }
        return start;
  ...
  static int consumeParentheses(CharSequence s, int start) {
    ...
    switch (s.charAt(start)) {
      case '(':
        ++start;
        while (start < s.length()) {
          if (s.charAt(start) == ')') return start + 1;
  ...
  public static ArrayList<CharSequence> splitSQL(CharSequence sql) {
    ...
    for (CharSequence c : new SQLSplitter(sql)) ret.add(c);
    return ret;
  }
