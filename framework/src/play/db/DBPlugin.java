...
public class DBPlugin extends PlayPlugin {
  ...
  protected DataSourceFactory factory(Configuration dbConfig) {
    String dbFactory =
        dbConfig.getProperty("db.factory", "play.db.hikaricp.HikariDataSourceFactory");
    try {
      ...
      try {
        ...
        if (e.getCause() instanceof InterruptedException) {
          throw new DatabaseException(
              "Cannot connected to the database[" + dbName + "]. Check the configuration.", e);
        }
        throw new DatabaseException(
            "Cannot connected to the database[" + dbName + "], " + e.getMessage(), e);
      }
    }
  }
  ...
  private boolean changed() {
    ...
    for (String dbName : dbNames) {
      ...
      if ((datasourceName.startsWith("java:") || datasourceName.startsWith("jndi:"))
          && dbConfig.getProperty("db.url") == null) {
        if (ds == null) {
          return true;
        }
      } else {
