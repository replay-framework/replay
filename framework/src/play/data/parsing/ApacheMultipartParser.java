...
public class ApacheMultipartParser extends DataParser {
  ...
  public static class AutoFileItem implements FileItem {
    ...
    public AutoFileItem(FileItemStream stream) {
      ...
      this.sizeThreshold =
          Integer.parseInt(Play.configuration.getProperty("upload.threshold", "10240"));
      this.repository = null;
    }
    // ------------------------------- Methods from javax.activation.DataSource

    /**
     * Returns an {@link InputStream InputStream} that can be used to retrieve the contents of the
     * file.
     *
     * @return An {@link InputStream InputStream} that can be used to retrieve the contents of the
     *     file.
     ...
     * disk file. Although this storage will be deleted automatically when the {@code FileItem}
    ...
    /**
     * Determines whether or not a {@code FileItem} instance represents a simple form field.
     *
    ...
    /**
     * Returns an {@link OutputStream OutputStream} that can be used for storing the contents of the
     * file.
     *
     * @return An {@link OutputStream OutputStream} that can be used for storing the contents of the
     *     file.
     */
    @Override
    ...
    /**
     * Returns the {@link File} object for the {@code FileItem}'s data's temporary location on the
     * disk. Note that for {@code FileItem}s that have their data stored in memory, this method will
     * return {@code null}. When handling large files, you can use {@link File#renameTo(File)} to
     * move the file to new location without copying the data, if the source and destination
    ...
    /**
     * Creates and returns a {@link File File} representing a uniquely named temporary file in the
     * configured repository path. The lifetime of the file is tied to the lifetime of the {@code
     * FileItem} instance; the file will be deleted when the instance is garbage collected.
     *
  ...
  public Map<String, String[]> parse(Request request) {
    ...
    try {
      FileItemIteratorImpl iter =
          new FileItemIteratorImpl(
              request.body, request.headers.get("content-type").value(), request.encoding);
      while (iter.hasNext()) {
        ...
        try {
          ...
          if (fileItem.isFormField()) {
            ...
            if (uploads == null) {
              ...
              request.args.put("__UPLOADS", uploads);
            }
            try {
  ...
  // ----------------------------------------------------------- Data members
  /**
   * The maximum size permitted for the complete request, as opposed to {@link #maxFileSize}. A
   * value of -1 indicates no maximum.
   */
  private final long maxRequestSize =
      Integer.parseInt(Play.configuration.getProperty("upload.maxRequestSize", "-1"));
  /**
   * The maximum size permitted for a single uploaded file, as opposed to {@link #maxRequestSize}. A
   * value of -1 indicates no maximum.
   */
  private final long maxFileSize =
      Integer.parseInt(Play.configuration.getProperty("upload.maxFileSize", "-1"));
  ...
  /**
   * Retrieves the file name from the {@code Content-disposition} header.
   *
  ...
  /**
   * Retrieves the field name from the {@code Content-disposition} header.
   *
   * @param headers A {@code Map} containing the HTTP request headers.
  ...
  private String getFieldName(Map<String, String> headers) {
    ...
    String cd = getHeader(headers, CONTENT_DISPOSITION);
    if (cd != null
        && (cd.toLowerCase().startsWith(FORM_DATA) || cd.toLowerCase().startsWith(ATTACHMENT))) {
   ...
   * @return The value of specified header, or a comma-separated list if there were multiple headers
   *     of that name.
   */
  ...
  private class FileItemIteratorImpl implements FileItemIterator {
    ...
    private class FileItemStreamImpl implements FileItemStream {
      ...
      FileItemStreamImpl(String pName, String pFieldName, String pContentType, boolean pFormField) {
        ...
        if (maxFileSize != -1) {
          istream =
              new LimitedInputStream(istream, maxFileSize) {
     ...
     * @throws IOException An I/O error occurred.
     */
    FileItemIteratorImpl(InputStream input, String contentType, Charset charEncoding)
      ...
      if (maxRequestSize >= 0) {
        ...
        input =
            new LimitedInputStream(input, maxRequestSize) {
      ...
      for (; ; ) {
        ...
        if (currentFieldName == null) {
          ...
          if (fieldName != null) {
            ...
            currentItem =
                new FileItemStreamImpl(
                    fileName, fieldName, getHeader(headers, CONTENT_TYPE), fileName == null);
          ...
          if (fileName != null) {
            currentItem =
                new FileItemStreamImpl(
                    fileName, currentFieldName, getHeader(headers, CONTENT_TYPE), false);
            itemValid = true;
            return true;
          }
        }
     ...
     * @throws NoSuchElementException No more items are available. Use {@link #hasNext()} to prevent
     *     this exception.
     * @throws IOException Reading the file item failed.
     */
    @Override
  ...
  private static class FileUploadIOException extends IOException {
    ...
    /**
     * The exceptions cause; we overwrite the parent classes field, which is available since Java
     * 1.4 only.
     */
     ...
     */
    private FileUploadIOException(FileUploadException pCause) {
  ...
  private static class InvalidContentTypeException extends FileUploadException {
    ...
    /**
     * Constructs an {@code InvalidContentTypeException} with the specified detail message.
     *
  ...
  private static class IOFileUploadException extends FileUploadException {
     ...
     */
    private IOFileUploadException(String pMsg, IOException pException) {
      super(pMsg);
      cause = pException;
    }
  ...
  private static class SizeLimitExceededException extends SizeException {
    ...
    /**
     * Constructs a {@code SizeExceededException} with the specified detail message, and actual and
     * permitted sizes.
     *
     * @param message The detail message.
     */
