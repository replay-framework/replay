  ...
  public FileUpload(FileItem fileItem) {
    ...
    if (fileItem.getName().trim().isEmpty()) {
      ...
      defaultFile =
          new File(
              tmp,
              FilenameUtils.getName(fileItem.getFieldName())
                  + File.separator
      ...
      try {
        if (!defaultFile.getCanonicalPath().startsWith(tmp.getCanonicalPath())) {
          throw new IllegalStateException(
              "Temp file "
                  ...
                  + " try to override existing file "
                  + defaultFile.getCanonicalPath());
        }
        ...
        throw new IllegalStateException(
            "Error when trying to write to file " + defaultFile.getAbsolutePath(), e);
      }
    }
  }
